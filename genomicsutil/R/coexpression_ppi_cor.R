...
coexpression_ppi_cor <- function (net, gene_annot = NULL, symbol_col = "gene_name", directed = FALSE, 
                                  string_version = "10", species = 9606, string_dir = "", max_homology_bitscore = Inf, 
                                  benchmark_pathway = NULL) 
{
  ...
  # get ppi scores
  known_ppis_df = get_string_ppi(genes = bg, directed = directed, 
                                 string_version = string_version, species = species, score_threshold = 0, 
                                 max_homology_bitscore = max_homology_bitscore, benchmark_pathway = benchmark_pathway, 
  ...
  ppi_score_mat = ppi_score_mat/1000  # convert to probability between 0 and 1
  
  # correlation test
  ppi_score_values = ppi_score_mat[lower.tri(net, diag = F)]
